...
export default function LandingPage() {
  ...
  return (
    <div style={{ fontFamily: "system-ui, sans-serif", background: "#060d1a", color: "#f0f6ff", minHeight: "100vh" }}>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .lp-logo { display: flex; align-items: center; gap: 8px; font-size: 1.05rem; font-weight: 700; color: #f0f6ff; cursor: pointer; text-decoration: none; }
        .lp-logo-mark { width: 26px; height: 26px; background: #3b82f6; border-radius: 6px; display: flex; align-items: center; justify-content: center; }
        .lp-nav-right { display: flex; align-items: center; gap: 1rem; }
        .lp-nav-link { color: #64788f; font-size: 0.875rem; text-decoration: none; transition: color 0.2s; }
        .lp-nav-link:hover { color: #f0f6ff; }
        .lp-nav-btn { background: #3b82f6; color: #fff; padding: 7px 16px; border-radius: 6px; font-size: 0.85rem; font-weight: 600; text-decoration: none; border: none; cursor: pointer; transition: opacity 0.2s; }
        .lp-nav-btn:hover { opacity: 0.85; }
        .lp-hero {
          min-height: 100vh; display: flex; flex-direction: column;
          align-items: center; justify-content: center;
          padding: 80px 5vw 60px; text-align: center;
          background: radial-gradient(ellipse 80% 50% at 50% 20%, rgba(59,130,246,0.07) 0%, transparent 70%);
        }
        .lp-hero h1 { font-size: clamp(2rem, 5vw, 3.4rem); font-weight: 800; line-height: 1.15; letter-spacing: -1px; margin-bottom: 1.25rem; max-width: 680px; }
        ...
        .lp-btn-ghost:hover { border-color: rgba(59,130,246,0.5) !important; }
        ...
        .lp-pkg-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 1rem; }
        .lp-pkg-card { background: rgba(11,22,40,0.8); border: 1px solid rgba(59,130,246,0.15); border-radius: 12px; padding: 1.5rem; position: relative; transition: border-color 0.2s, transform 0.2s; }
        .lp-pkg-card:hover { border-color: rgba(59,130,246,0.4); transform: translateY(-2px); }
        .lp-pkg-card.popular { border-color: #3b82f6; }
        .lp-popular-tag { position: absolute; top: -1px; left: 50%; transform: translateX(-50%); background: #3b82f6; color: #fff; font-size: 0.68rem; font-weight: 700; padding: 3px 10px; border-radius: 0 0 6px 6px; text-transform: uppercase; letter-spacing: 0.5px; white-space: nowrap; }
        .lp-pkg-speed { font-size: 0.72rem; color: #60a5fa; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 4px; }
        .lp-pkg-name { font-size: 1.05rem; font-weight: 700; margin-bottom: 0.75rem; }
        .lp-pkg-price { display: flex; align-items: baseline; gap: 2px; margin-bottom: 4px; }
        .lp-pkg-currency { font-size: 0.85rem; color: #64788f; }
        .lp-pkg-amount { font-size: 1.9rem; font-weight: 800; line-height: 1; }
        .lp-pkg-period { font-size: 0.75rem; color: #64788f; margin-left: 2px; }
        .lp-pkg-validity { font-size: 0.75rem; color: #64788f; margin-bottom: 0.75rem; }
        .lp-pkg-desc { font-size: 0.8rem; color: #64788f; line-height: 1.5; margin-bottom: 1.25rem; min-height: 32px; }
        .lp-pkg-btn { display: block; width: 100%; text-align: center; padding: 9px; border-radius: 7px; font-size: 0.85rem; font-weight: 600; text-decoration: none; border: 1px solid rgba(59,130,246,0.35); color: #60a5fa; background: transparent; cursor: pointer; transition: background 0.2s, color 0.2s; }
        .lp-pkg-btn:hover, .lp-pkg-card.popular .lp-pkg-btn { background: #3b82f6; color: #fff; border-color: #3b82f6; }
        .lp-zones-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(175px, 1fr)); gap: 0.75rem; }
        .lp-zone-card { background: #0f1e36; border: 1px solid rgba(59,130,246,0.15); border-radius: 10px; padding: 1rem 1.25rem; display: flex; align-items: flex-start; gap: 10px; }
        .lp-zone-dot { width: 8px; height: 8px; border-radius: 50%; background: #22d3a0; flex-shrink: 0; margin-top: 5px; }
        ...
        .lp-cta { background: #0b1628; border-top: 1px solid rgba(59,130,246,0.15); border-bottom: 1px solid rgba(59,130,246,0.15); padding: 70px 5vw; text-align: center; }
        ...
        .lp-spinner { display: inline-block; width: 20px; height: 20px; border: 2px solid rgba(59,130,246,0.2); border-top-color: #3b82f6; border-radius: 50%; animation: lp-spin 0.8s linear infinite; margin-bottom: 0.75rem; }
        ...
        .lp-foot-links a { font-size: 0.78rem; color: #64788f; text-decoration: none; }
        .lp-foot-links a:hover { color: #60a5fa; }
      ...
      <section className="lp-hero">
        ...
        <p>Choose a plan that fits your needs. No hidden fees, no throttling — just internet that works.</p>
        <div className="lp-btns">
          <a href="#packages" className="lp-btn lp-btn-blue">View packages</a>
      ...
      <section id="packages" className="lp-section-alt">
        ...
        <p className="lp-sec-sub">All packages include 24/7 support and access to your customer portal.</p>

        {pkgLoading ? (
          <div className="lp-loading"><div className="lp-spinner" /><br />Loading packages...</div>
        ) : packages.length === 0 ? (
          <div className="lp-loading">No packages available.</div>
        ) : (
            ...
            {packages.map((p, i) => {
              ...
              return (
                <div key={p.id} className={`lp-pkg-card${popular ? " popular" : ""}`}>
                  ...
                  <div className="lp-pkg-speed">{p.speedMbps} Mbps</div>
                  ...
                  {p.validity && <div className="lp-pkg-validity">{p.validity}</div>}
                  {p.description && <div className="lp-pkg-desc">{p.description}</div>}
                  <button className="lp-pkg-btn" onClick={() => navigate("/login")}>Get started</button>
                </div>
              );
            })}
          </div>
      ...
      <footer className="lp-footer">
        <div className="lp-logo" style={{ fontSize: "0.95rem" }}>
        ...
        <ul className="lp-foot-links">
          ...
          <li><button style={{ background: "none", border: "none", cursor: "pointer", color: "#64788f", fontSize: "0.78rem" }} onClick={() => navigate("/login")}>Login</button></li>
        </ul>
